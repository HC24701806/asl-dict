    ...
    for i in range(10):
        {
            ...
            .output(f'../augmented_dataset/{id}_{i + 1}.mp4', loglevel='quiet').run(overwrite_output=True)
        }
...
with open('mini_dataset.csv') as in_csv, open('augmented_mini_dataset.csv', 'w') as out_csv:
    ...
    for line in reader:
        split = line[0]
        id = line[1]
        ffmpeg.input(f'../ASL_Citizen/videos/{id}.mp4').output(f'../augmented_dataset/{id}.mp4', loglevel='quiet').run(overwrite_output=True)
